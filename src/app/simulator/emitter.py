...
def emit_mote(mote_id: int, df, producer, speed_factor: float = 100.0, stop_event: Optional[object] = None) -> None:
    ...
    - `df` is expected to contain columns: `timestamp`, `epoch`, `temperature`,
      `humidity`, `light`, `voltage`, and optionally `updated_timestamp` (pre-mapped).
    ...
    for _, row in df.iterrows():
        ...
        # Use pre-calculated updated_timestamp if available, otherwise calculate at runtime
        import pandas as pd
        if 'updated_timestamp' in row.index and pd.notna(row.get('updated_timestamp')):
            # Use pre-mapped timestamp from data file (preferred for accuracy)
            # Convert pandas Timestamp to datetime if needed
            updated_ts = pd.to_datetime(row['updated_timestamp']).to_pydatetime()
        else:
            # Fallback: Calculate updated timestamp mapping 2004 data to current year
            # Handle leap year edge case (Feb 29 in leap year -> Feb 28 in non-leap year)
            try:
